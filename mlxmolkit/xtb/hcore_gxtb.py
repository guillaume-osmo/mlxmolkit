...
def build_hcore_gxtb(
    ...
    *,
    ...
    ``carbon_plevel`` toggles :func:`_carbon_plevel_shift`, the oracle-fitted
    carbon-p patch. It is a stand-in for a missing term, not g-xTB physics, so
    it has to be switchable: once the real term is in place the patch would
    double-count. Default True preserves the historical behaviour.
    """
