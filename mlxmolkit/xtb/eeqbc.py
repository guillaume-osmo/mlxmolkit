...
def eeqbc_solve(
    ...
    *,
    ...
    atoms = np.asarray(atomic_numbers, dtype=np.intp)
    # ⚠️ BOHR.  Every radius in EEQBC2025_PARAMS is atomic units -- cov_radii
    # is H 0.54368, C 1.27782, O 1.27356, which are bohr, not the 0.31 / 0.76 /
    # 0.66 angstrom values -- and the whole module compared them against
    # angstrom coordinates.  Measured against the binary's own
    # `get_eeqbc_charges` on H2O: angstrom gives max|d| 8.134e-02, bohr gives
    # 4.938e-07.
