...
#: between v2.0.0 and v2.0.1 (one changed function, ``basis_qvszp::get_qeff``).
#: Verified bit-exact against the binary by
#: gxtb-recovery/probes/get_qeff/test_get_qeff.py.
QEFF_CN_EPS = 1.0e-12
QEFF_CN_DELTA = 1.0e-6
...
def qvszp_qeff(q: np.ndarray, cn: np.ndarray, k0: np.ndarray, k1: np.ndarray, k2: np.ndarray, k3: np.ndarray) -> np.ndarray:
    ...
    Matches ``basis_qvszp::get_qeff`` of the g-xTB **v2.0.1** binary:

        qeff = k0*(q - k1*q**2) + k2*(sqrt(cn + 1e-12) - 1e-6) + k3*cn*q
    """

    q_arr = np.asarray(q, dtype=np.float64)
    cn_arr = np.asarray(cn, dtype=np.float64)
    sqcn = np.sqrt(cn_arr + QEFF_CN_EPS)
    return k0 * (q_arr - k1 * q_arr * q_arr) + k2 * (sqcn - QEFF_CN_DELTA) + k3 * cn_arr * q_arr
...
def qvszp_qeff_derivatives(
    ...
    q_arr = np.asarray(q, dtype=np.float64)
    cn_arr = np.asarray(cn, dtype=np.float64)
    # v2.0.1 differentiates sqrt(cn + eps), so no ad-hoc clamp is needed: the
    # epsilon is what keeps this finite at cn == 0.  The binary writes both
    # derivatives or neither.
    sqcn = np.sqrt(cn_arr + QEFF_CN_EPS)
    dq = k0 * (1.0 - 2.0 * k1 * q_arr) + k3 * cn_arr
    dcn = 0.5 * k2 / sqcn + k3 * q_arr
    return dq, dcn
...
def build_gxtb_qvszp_basis(
    ...
    *,
    ...
    if cn is None:
        ...
        # COPY, never alias.  `basis.cn` is written in place downstream
        # (`_restore_calc_cn` swaps the basis CN for the calculator's), so
        # aliasing the caller's array silently rewrites it -- which turns any
        # "frozen cn" finite difference into a moving one.
        cn_arr = np.array(cn, dtype=np.float64, copy=True)
