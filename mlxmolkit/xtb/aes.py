...
def fockelectro(
    ...
    # `add_vmp_to_h1` (recovered) writes the two multipole terms with
    # DIFFERENT integrals:
    #     h1(i,j) -= 0.5*( sum_k vmp(k,at(j))*mpint(k,i,j)
    #                    + sum_k vmp(k,at(i))*mpint(k,j,i) )
    # because the binary's dpint/qpint are ATOM-CENTRED about the column atom,
    # so mpint(i,j) != mpint(j,i).
    #
    # ⚠️ That is NOT a defect here, and the split form below is written out
    # only so the difference is visible.  This module's integrals are about a
    # single GLOBAL origin and are therefore exactly symmetric -- measured,
    # |mpint[k] - mpint[k].T| = 0.0 for all 3 dipole and all 6 quadrupole
    # components -- so the two forms coincide bit for bit and the port is
    # internally consistent in the other convention.  `port_divergence` stage 7
    # already converts between them (`dp_conv = dp - R_atom*S`) and passes.
    fji = S * (vs_ao[None, :] + vs_ao[:, None])

    for k in range(3):
        vd_ao = vd[k, ao]
        fji += dpint[k].T * vd_ao[:, None] + dpint[k] * vd_ao[None, :]

    # vq is in qpint layout (xx, yy, zz, xy, xz, yz), same as qpint.
    for k in range(6):
        vq_ao = vq[k, ao]
        fji += qpint[k].T * vq_ao[:, None] + qpint[k] * vq_ao[None, :]
