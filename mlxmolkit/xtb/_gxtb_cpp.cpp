...
PyObject* repulsion_energy_gradient(PyObject*, PyObject* args) {
  ...
  for (npy_intp i = 0; i < n; ++i) {
    for (npy_intp j = i + 1; j < n; ++j) {
      ...
      // The binary's `get_energy` is
      //     vrep = 0.5 * mat @ scaled_zeff ;  E_i = vrep_i * scaled_zeff_i
      // i.e. E = 0.5 * sum_ij mat_ij z_i z_j = sum_{i<j} mat_ij z_i z_j,
      // and THIS LOOP ALREADY RUNS ONLY OVER i < j.  The 2.0 that used to
      // be here double-counted every pair: the energy came out exactly
      // twice the binary's on H2O, CH3SH and benzene alike.  `matvec` is
      // untouched -- the POTENTIAL path wants the un-halved `mat @ z` and
      // already matches the binary at 1e-18.
      const double pair_scale = scaled_data[i] * scaled_data[j];
...
PyMethodDef methods[] = {
    ...
    },
    {
        ...
        repulsion_pair_value_asm,
        METH_VARARGS,
        "Compute the g-xTB repulsion pair form with rvdw/R polynomial terms.",
    },
    {
        "repulsion_pair_value_asm_deriv",
        repulsion_pair_value_asm_deriv,
        METH_VARARGS,
        "Compute the g-xTB repulsion pair form and d/dR derivative.",
    },
    {
    ...
    },
    {
        ...
        repulsion_pair_matrix_asm,
        METH_VARARGS,
        "Build a symmetric pair matrix from the g-xTB repulsion pair form.",
    },
    {
        "repulsion_energy_gradient_asm",
        repulsion_energy_gradient_asm,
        METH_VARARGS,
        "Compute energy, gradient, and matvec from the g-xTB pair form.",
    },
    {
