...
GXTB_MFX_OMEGA = 0.2
# 🔑 1.0, not 1.3826597204.  That value is the ON-SITE scalar; the
# binary keeps it one slot earlier in the same constant pool and the
# two were swapped here.
GXTB_MFX_GEXP = 1.0
...
# --- Mulliken Fock-exchange kernel, transcribed from the recovered binary ----
#
# The previous implementation followed public tblite's ``mulliken-FX``.  The
# released g-xTB binary does something different, and the four differences are
# each worth naming because three of them are invisible in a total energy and
# all four are angular-momentum structured -- which is why they showed up as a
# sulfur d-shell population 3.6x the binary's.
#
# The specification is ``exchange_fock::get_gmulliken_0d`` in the recovery
# tree, verified against the shipped routine at 0 ulp:
#
#     gpar(ish) = ps_fock_shell_hubbard(ish, Z) * pa_hubbard_parameter(Z)
#     xi(ish)   = ps_fock_avg_exp(ish, Z)
#     fpar(ish) = pg_fock_offdiag_l(l(ish) + 1)        [0.05, 2.5, 2.5, 2.5]
#     gam       = general_average(gpar_i, gpar_j; max(xi_i, xi_j))
#     favg      = sqrt(fpar_i * fpar_j)
#     rad       = vdW PAIR radius * aatoau * mean(pa_rvdw_scale_i, _j)
#     damp      = exp(-(r * (c0 + rad * c1)))
#     gab       = ((favg / damp) * gam) ** (-gexp)
#     off-site  = (alpha + erf(r*omega)*lrscale) / (r**gexp + gab)**(1/gexp)
#     on-site   diagonal:     gam * onsite * alpha
#               off-diagonal: favg * gam * alpha
#
# What was wrong before:
#
#   * ``favg`` was absent entirely.  It is the ONLY strongly l-dependent,
#     element-independent factor in the model, and dropping it flattens a 50x
#     s:p contrast to nothing.
#   * ``gexp`` was 1.3826597204.  That number is the ON-SITE scalar, which
#     lives one slot earlier in the binary's constant pool; ``gexp`` is 1.0.
#   * ``pa_hubbard_parameter`` was dropped from ``gpar``, leaving gamma 2.1x
#     to 5.8x too large, per element.
#   * the damping was absent, and there is no separate fitted on-site
#     constant -- the on-site branch is a different formula, not a limit of
#     the off-site one.
GXTB_MFX_ONSITE = 1.3826597204
GXTB_MFX_C0 = -0.2960502355
GXTB_MFX_C1 = 0.018511538388678535
# `pa_rvdw_scale`'s default entry is exactly 1/aatoau, so for elements that
# carry it `rad` is the raw vdW pair radius; keeping the reciprocal exact
# preserves that identity.
_MFX_AATOAU = 1.0 / 0.5291772109044924
...
def _mfx_gamma_ao(
    ...
    *,
    ...
    """Shell-resolved Mulliken Fock-exchange kernel, expanded to AO pairs."""
    ...
    nsh = shell_atom.size
    gpar = np.zeros(nsh, dtype=np.float64)
    xi = np.ones(nsh, dtype=np.float64)
    fpar = np.zeros(nsh, dtype=np.float64)
    for ish in range(nsh):
        Z = int(atoms[int(shell_atom[ish])])
        ls = int(shell_local[ish])
        gpar[ish] = (float(GXTB_PARAMS["ps_fock_shell_hubbard"][Z - 1, ls])
                     * float(GXTB_PARAMS["pa_hubbard_parameter"][Z - 1]))
        xi[ish] = float(GXTB_PARAMS["ps_fock_avg_exp"][Z - 1, ls])
        fpar[ish] = float(GXTB_PARAMS["pg_fock_offdiag_l"][int(shell_l[ish])])
        if gpar[ish] <= 0.0:
            gpar[ish] = 1.0e-12

    scale = np.asarray(GXTB_PARAMS["pa_rvdw_scale"], dtype=np.float64)[atoms - 1]
    rad = (mctc_vdw_pair_matrix_bohr(atoms) * _MFX_AATOAU
           * (0.5 * (scale[:, None] + scale[None, :])))

    onsite = GXTB_MFX_ONSITE
    c0, c1 = GXTB_MFX_C0, GXTB_MFX_C1
    alpha = float(frscale)

    kmat = np.zeros((nsh, nsh), dtype=np.float64)
    for ish in range(nsh):
        iat = int(shell_atom[ish])
        for jsh in range(ish + 1):
            jat = int(shell_atom[jsh])
            gam = _generalized_hubbard_average(
                float(gpar[ish]), float(gpar[jsh]),
                max(float(xi[ish]), float(xi[jsh])))
            favg = math.sqrt(float(fpar[ish]) * float(fpar[jsh]))
            if iat != jat:
                r1 = float(np.linalg.norm(coords_bohr[iat] - coords_bohr[jat]))
                rg = r1 ** float(gexp)
                damp = math.exp(-(r1 * (c0 + float(rad[iat, jat]) * c1)))
                gab = ((favg / damp) * gam) ** (-float(gexp))
                den = (rg + gab) ** (1.0 / float(gexp))
                val = (alpha + math.erf(r1 * float(omega)) * float(lrscale)) / den
            elif ish == jsh:
                val = gam * onsite * alpha
            else:
                val = favg * gam * alpha
            kmat[ish, jsh] = val
            kmat[jsh, ish] = val

    return kmat[bf_to_shell][:, bf_to_shell]
...
def _first_order_offsite(
    ...
    🔴 MEASURED BROKEN — do not enable expecting physics.  The potential here is
...
def gxtb_energy(
    ...
    *,
    ...
    ==================  =======  =======  =======  =======
    (no MFX)            0.04617  0.05541  0.15704     0.37
    **default (MFX)**   0.02494  0.03420  0.10486     0.37
    MFX + AES           0.02062  0.02943  0.11016     1.61
    MFX + AES + ACP     0.02175  0.02663  0.07674     1.59
    MFX + two-body 3rd  0.02283  0.03291  0.11568     0.37
    ==================  =======  =======  =======  =======

    ``use_mfx_exchange`` (SI Eq. 153 range-separated Mulliken Fock exchange)
    defaults to True: it is the single largest missing charge term, it wins on
    every one of the 20 molecules individually, and it is **free** — same
    0.37 s/mol as without it.

    ``use_aes`` is the best accuracy lever on top of that (-17% MAE) but costs
    4.3x wall time, so it is left to the caller.  Prefer ``use_aes=True`` when
    generating features where charge fidelity dominates cost; leave it off for
    bulk data generation.  Note AES only became measurable at all once the
    one-centre exchange table shipped (#80) — before that it, ``use_aniso_h0``
    and ``use_twobody_third_order`` all died on a missing file.

    Measured and *not* worth enabling:

    * ``use_aniso_h0`` is worse than doing nothing (0.05051 vs 0.04617 without
      MFX; 0.02972 vs 0.02494 with it), and costs 3x.
    * ``use_acp_hamiltonian`` depends on what else is on, so beware simple
      reasoning here.  Without AES it *worsens* MAE (0.02494 -> 0.03058).  With
      AES it trades mean for tail: MAE 0.02062 -> 0.02175, but the best RMSE
      (0.02663) and much the best worst-case error (0.11016 -> 0.07674), because
      it nearly eliminates the oxygen residual (+0.0545 -> +0.0146 e).  Choose it
      when outliers matter more than the mean.
    * dropping ``use_carbon_plevel_shift`` costs 2.6x (0.02494 -> 0.06408), so
      that oracle-fitted patch is *not* merely standing in for MFX.

    🔴 Water is a misleading probe for all of this: it made ACP look like the
    missing piece (it repairs the O s-shell population from 1.6916 to 1.8202
    against the binary's 1.8050) when across real molecules it is a wash.
    ...
    H_eht, shell_self = build_hcore_gxtb(
        atoms, coords, basis, carbon_plevel=use_carbon_plevel_shift
    )
