...
from .two_elec_two_center_int_local_frame_np import two_elec_two_center_int_local_frame as TETCILF
from .two_elec_two_center_int_local_frame_d_orbitals_np import two_elec_two_center_int_local_frame_d_orbitals as TETCILFDO
from .cal_par_np import *
from .constants_np import ev
import sys
import numpy

# import scipy.special
from .parameters_np import  PWCCT
from .RotationMatrixD_np import *
import time
...
#    x = POIJ(2,y*math.sqrt(2),hpp[isY])
    ...
    rho0aTMP = rho_0[idxi].copy()
    rho0bTMP = rho_0[idxj].copy()
...
#            ((nj <= 12) | ((nj >= 18) & (nj <=20)) | ((nj >= 30) & (nj <= 32)) | ((nj >= 36) & (nj <= 38)) | ((nj >= 48) & (nj <= 50)) | ((nj >= 54) & (nj <= 56)) | ((nj >= 80) & (nj <= 83))) & (nj !=1)
 ...
 #   else:
    ...
    riXHPM6 = torch.clone(riXH)
    riPM6a = torch.clone(ri)
    riPM6b = torch.clone(ri)
    if(rho_corea.sum() > 0 or rho_coreb.sum() > 0 ):
       rho0aTMP = torch.clone(rho0a)
       rho0bTMP = torch.clone(rho0b)
       A = (rho_corea != 0.000)
       B = (rho_coreb != 0.000)
    ...
    #
    #
    # yXH=torch.zeros(xXH.shape[0],2,dtype=dtype)
    ...
    #
    # wXH = torch.zeros(riXH.shape[0],10,dtype=dtype)
    # # (s s/s s)
    # wXH[...,1-1] = riXH[...,1-1]
    # ## (px s/s s)
    # #w[2] = ri[2]*x[1]
    ...
    # x=-xij[K]
    # # x[...,0] +=eps 
    # y=torch.zeros(x.shape[0],2,dtype=dtype)
    ...
    #
    # e1b = torch.zeros((rij.shape[0],4,4),dtype=dtype)
    ...
    if themethod in ("PM6", "PM7"):
        ...
        e1bD = torch.zeros((rij.shape[0],9,9),dtype=dtype)
        ...
        wc  = torch.zeros(rij.shape[0],45,45,dtype=dtype)
    ...
    wc  = torch.zeros(rij.shape[0],10,10,dtype=dtype)
    wc[HH,0,0] = wHH
    wc[XH,:,0] = wXH
...
def w_withquaternion(mol,tore,ni, nj, xij, riXH, ri, wHH):
    ...
    ri_s    = tuple(np.moveaxis(ri, -1, 0))    # tuple of length 22
    riXH_s  = tuple(np.moveaxis(riXH, -1, 0))  # tuple of length 4
...
def rotate_with_quaternion(v,calculate_gradient=False):
    ...
    q_raw = torch.cat((u, torch.unsqueeze(w_, -1)), dim=-1)
    ...
    # q_raw = torch.where(torch.unsqueeze(mask, -1), base_q, q_raw)
    q_raw[mask] = torch.tensor([0.0, 0.0, 1.0, 0.0],dtype=dtype)#.unsqueeze(0)
    ...
    rot = torch.empty((n, 3, 3), dtype=dtype)
    ...
    dq_raw_dv = torch.zeros((n, 4, 3), dtype=dtype)
    dq_raw_dv[:, 1, 2] = 1.0   # ∂u_y/∂v_z
    dq_raw_dv[:, 2, 1] = -1.0  # ∂u_z/∂v_y
    dq_raw_dv[:, 3, 0] = 1.0   # ∂w/∂v_x
    # zero out masked rows
    dq_raw_dv = dq_raw_dv * (~torch.unsqueeze(mask), -1).unsqueeze(-1)

    # dN/dv_j = (1/N) ∑_i q_raw_i * ∂q_raw_i/∂v_j
    dN_dv = (torch.unsqueeze(q_raw, -1) * dq_raw_dv).sum(dim=1, keepdim=False) / N

    # dq/dv from quotient rule: ∂(q_raw_i/N)/∂v_j
    dq_dv = (dq_raw_dv * torch.unsqueeze(N, -1) - 
             torch.unsqueeze(q_raw, -1) * torch.unsqueeze(dN_dv, 1)) / (torch.unsqueeze(N, -1)**2)

    dr_dq = torch.zeros((n, 3, 3, 4), dtype=dtype)
