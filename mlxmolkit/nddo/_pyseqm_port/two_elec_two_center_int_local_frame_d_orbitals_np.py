...
import numpy
import math
from .RotationMatrixD_np import RotateCore
import time
...
##    t = time.time()
    ...
    if(YH.sum() > 0 ):
        ...
        ##coreYY = torch.zeros(YY.sum(),90,dtype=dtype)
    ...
    ##    coreYY[...,88] = 0
    ##    coreYY[...,89] = coreYY[...,75]
        riYH =  torch.zeros(YH.sum(),45,dtype=dtype)
...
#                      ev1/torch.sqrt((r0[YH])**2+1/2*(dsb0[YH])**2+1/2*(dda0[YH])**2+rhoSDDD)
        ...
        ##print(riYH)
        ##sys.exit()
        coreYH = torch.zeros(YH.sum(),46,dtype=dtype)
        coreYHLocal = torch.zeros(YH.sum(),46,dtype=dtype)
        ##print(r0[YH]**2,rhoSS)
        ##print(ni,nj)
...
#        LL = L.bool()
    ...
    if(YX.sum() > 0 ):
        ...
        ##coreYX = torch.zeros(YX.sum(),90,dtype=dtype)
        ##coreYXLocal = torch.zeros(YX.sum(),90,dtype=dtype) 
    ...
    ##    coreYX[...,88] = 0
    ##    coreYX[...,89] = coreYX[...,75]
        riYX =  torch.zeros(YX.sum(),450,dtype=dtype)
...
#        print(RotateCore(coreYXLocal[...,:10],rotationMatrix[YX],2))
    ...
    ##    coreYY[...,88] = 0
    ##    coreYY[...,89] = coreYY[...,75]
        riYY =  torch.zeros(YY.sum(),2025,dtype=dtype)
