...
def test_gxtb_native_kernel_status_is_symbol_anchored():
    kernels = native_kernel_status()
...
@pytest.mark.skipif(not CPP_AVAILABLE, reason="C++ g-xTB extension is not built")
def test_repulsion_state_applies_reference_tables_and_formulas():
    atomic_numbers = np.array([1, 6, 8, 16], dtype=np.intp)
...
def test_repulsion_energy_gradient_matches_matrix_and_fd():
    ...
    # ⚠️ THE HALF IS THE BINARY'S.  `gxtb_repulsion::get_energy` is
