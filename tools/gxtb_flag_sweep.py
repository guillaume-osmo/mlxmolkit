...
COMBOS = [
    ...
    #
    # `use_first_order_offsite` does nothing on its own: gxtb_energy gates it as
    # `if use_first_order and use_first_order_offsite`. Every "+offsite" arm here
    # used to pass it alone, so it silently re-measured "base" — verified
    # bit-identical (MAE 0.04617 both) by tools/gxtb_charge_oracle.py. Both flags
    # are set together now, which is also what first exposed the offsite term as
    # broken (it drives q(O) to -2.40 on water).
    ("base", {}),
    ...
    ("+first+twobody3", {"use_first_order": True, "use_twobody_third_order": True}),
    (
        "all_on",
        {
